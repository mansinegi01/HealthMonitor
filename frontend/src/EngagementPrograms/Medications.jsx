        ...
        return (
          <div key={med._id} style={{ background: "#181c27", border: `1px solid ${taken ? color + "40" : isOverdue ? "#ff6b6b40" : "#232840"}`, borderRadius: 14, padding: 16, transition: "border .3s" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <button onClick={() => handleToggle(med)} disabled={isLogging}
                ...
                {isLogging ? <span style={{ fontSize: 14 }}>⋯</span>
                  : taken ? <span style={{ color, fontSize: 16 }}>✓</span>
                  : isOverdue ? <span style={{ color: "#ff6b6b", fontSize: 14, fontWeight: 700 }}>!</span>
                  : null}
              </button>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: 12, color: "#8891b0", marginBottom: 8 }}>
                  {med.dose} · {med.type} ·{" "}
                  {taken ? <span style={{ color }}>Taken ✓</span>
                    : isOverdue ? <span style={{ color: "#ff6b6b" }}>Overdue !</span>
                    : <span style={{ color: "#f5c842" }}>Upcoming</span>}
                </div>
