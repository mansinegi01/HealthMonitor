...
function useMedNotifications(medsRef) {
  ...
  const tag = `med-${med._id}`;
  if (firedRef.current.has(tag)) return;

  firedRef.current.add(tag);

  console.log(`[MedReminder] 🔔 Showing notification for ${med.name}`);

  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.ready.then((registration) => {
      registration.showNotification(`💊 ${label}: ${med.name}`, {
        body: `Dose: ${med.dose} at ${med.scheduledTime}`,
        tag,
      });
    });
  } else if ("Notification" in window) {
    new Notification(`💊 ${label}: ${med.name}`, {
      body: `Dose: ${med.dose}`,
    });
  } else {
...
export default function Medications() {
 ...
 const testNotification = useCallback(() => {
  ...
  if ("serviceWorker" in navigator) {
    navigator.serviceWorker.ready.then((registration) => {
      registration.showNotification("💊 Test Reminder — MindNest", {
        body: "Notifications are working perfectly!",
        tag: "med-test",
      });
    });
  } else {
    new Notification("💊 Test Reminder — MindNest", {
